...
export const Appointments = ({ title, row = false, modelId = '', selectedClientId, handleDelete, loading = false, deleteSelected }) => {
			...
			<div className={`col-md-12 d-flex overflow-auto p-0  ${row ? `flex-row` : `flex-wrap`}`}>
				{getAllAppointement.map(appoint => (
					<div className="text-center">
						...
						<input type="button" className="btn btn-danger btn-sm" value={deleteSelected === appoint._id && loading ? 'Deleting' : 'Delete'} onClick={() => handleDelete(appoint._id)} />
					</div>
				))}
